...
#define MAX_BACKGROUND_PROCS 128
#define MAX_SHELL_ARGS 64
#define MAX_SHELL_STAGES 16

static pid_t background_pids[MAX_BACKGROUND_PROCS];

static void addBackgroundPid(pid_t pid)
{
   int i;

   for (i = 0; i < MAX_BACKGROUND_PROCS; i++)
   {
...
static int makeBackgroundCommand(char *dst, size_t dst_size, const char *src)
{
   size_t len;
...
static int parseShellArgs(char *line, char **args, int max_args)
{
   int argc = 0;
   char *token;
...
static int extractRedirect(char **args, char **out_file, int *append)
{
   int i;
   int j;
...
static void redirectOutputFile(char *out_file, int append)
{
   int flags;
   int fd;
...
static int runParsedCommand(char **args)
{
   char *out_file;
   int append;
   pid_t pid;
   ...
   if (extractRedirect(args, &out_file, &append) == -1)
   {
   ...
   if (pid == 0)
   {
      redirectOutputFile(out_file, append);
      execvp(args[0], args);
      perror("execvp");
      _exit(127);
   }
...
static int runPipeline(char **args)
{
   char **stages[MAX_SHELL_STAGES];
   pid_t pids[MAX_SHELL_STAGES];
   char *out_file;
   int append;
   int stage_count = 1;
   int i;
   ...
   for (i = 0; args[i] != NULL; i++)
   {
      if (strcmp(args[i], "|") == 0)
      {
         if (args[i + 1] == NULL || stage_count >= MAX_SHELL_STAGES)
         {
         ...
         args[i] = NULL;
         stages[stage_count++] = &args[i + 1];
      }
   }

   if (extractRedirect(stages[stage_count - 1], &out_file, &append) == -1)
   {
      fprintf(stderr, "redirect error: missing output file\n");
      return 1;
   }
   ...
   for (k = 0; k < stage_count; k++)
   {
      ...
      if (k < stage_count - 1 && pipe(pipefd) < 0)
      {
         perror("pipe");
         return 1;
      }
...
static int executeLab4ShellCommand(const char *command)
{
   char line[TUPLENAME_LEN];
   ...
   argc = parseShellArgs(line, args, MAX_SHELL_ARGS);
   if (argc < 0)
   {
   ...
   return runParsedCommand(args);
}

static int executeShellCapture(const char *command, char *output, size_t output_size)
{
   int pipefd[2];
   pid_t pid;
   ssize_t nread;
   ...
   if (pid == 0)
   {
      ...
      close(pipefd[1]);
      _exit(executeLab4ShellCommand(command));
   }

   close(pipefd[1]);
   while ((nread = read(pipefd[0], output + used, output_size - used - 1)) > 0)
   {
      used += (size_t)nread;
...
int initCommon(u_short port)
{
   ...
   tsh.space = NULL ;
   tsh.retrieve = NULL; // Deprecated. Ignore.
   tsh.queue_hd = tsh.queue_tl = NULL ; // Query queue
...
void OpShell()
{
   ...
   tsh_shell_ot out;
   char output[1024];
   char background_command[TUPLENAME_LEN];
   pid_t pid;
   int rc;
   ...
   if (makeBackgroundCommand(background_command, sizeof(background_command), in.command))
   {
      pid = fork();
      if (pid < 0)
      {
         out.status = htons((short int)FAILURE);
         out.error = htons((short int)TSH_ER_NOTUPLE);
         out.length = htonl(0);
      ...
      if (pid == 0)
      {
         _exit(executeLab4ShellCommand(background_command));
      }

      addBackgroundPid(pid);
      snprintf(output, sizeof(output), "Started background pid %ld\n", (long)pid);
      out.status = htons((short int)SUCCESS);
      out.error = htons((short int)TSH_ER_NOERROR);
      out.length = htonl(strlen(output) + 1);

      if (!writen(newsock, (char *)&out, sizeof(out)))
         return;

      writen(newsock, output, strlen(output) + 1);
      return;
   }

   rc = executeShellCapture(in.command, output, sizeof(output));

   out.status = htons((short int)(rc == 0 ? SUCCESS : FAILURE));
   out.error = htons((short int)(rc == 0 ? TSH_ER_NOERROR : TSH_ER_NOTUPLE));
   out.length = htonl(strlen(output) + 1);
...
void sigchldHandler()
{
   ...
   for (i = 0; i < MAX_BACKGROUND_PROCS; i++)
   {
      ...
      fp = fopen("tsh_status.log", "a");
      if (fp != NULL)
      {
         if (WIFEXITED(status))
            fprintf(fp, "pid %ld exited with status %d\n", (long)pid, WEXITSTATUS(status));
         else if (WIFSIGNALED(status))
            fprintf(fp, "pid %ld terminated by signal %d\n", (long)pid, WTERMSIG(status));
         else
            fprintf(fp, "pid %ld changed status\n", (long)pid);
         fclose(fp);
      }
