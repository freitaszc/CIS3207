...
void OpShell()
{
   ...
   getchar();
   printf("\nEnter shell command : ");
   fgets(out.command, sizeof(out.command), stdin);

   out.command[strcspn(out.command, "\n")] = '\0';
   if (strlen(out.command) == 0)
   {
      printf("\nNo command entered.\n");
      getchar();
      return;
   ...
   if (ntohs(in.status) == SUCCESS && ntohl(in.length) > 0)
   {
      buff = (char *)malloc(ntohl(in.length));
      if (!readn(tshsock, buff, ntohl(in.length)))
      {
         perror("\nOpShell::readn output\n");
         free(buff);
         getchar();
         return;
      }

      buff[ntohl(in.length) - 1] = '\0';
      printf("\noutput :\n%s\n", buff);
      free(buff);
   }
